...
public class MainMenu
{
    ...
    public MainMenu() {
        // Setting up my httpclient class I'm going to use to send http requests and adding the base url so I don't have to keep retyping it
    ...
    public async Task StartAsync() {
        ...
        while(true) {
            ...
            switch(input)
            {
                ...
                break;
                case "3":
                    // HttpResponseMessage msg = await _http.GetAsync("workouts");
                    // Console.WriteLine(await msg.Content.ReadAsStringAsync());

                    string content = await _http.GetStringAsync("workouts");
                    List<WorkoutSession> sessions = JsonSerializer.Deserialize<List<WorkoutSession>>(content);
    ...
    private async Task CreateNewWorkout() {
        ...
        try
        {
            JsonContent jsonContent = JsonContent.Create<WorkoutSession>(session);
            await _http.PostAsync("workouts", jsonContent);
